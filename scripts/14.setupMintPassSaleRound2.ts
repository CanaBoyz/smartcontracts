...
async function main() {
    log.header("Start MintPassCard Sale Round 2")
    const { mintPassCardSaleRoundParams2, cardSellerAddress } = meta.read()

    if (!mintPassCardSaleRoundParams2) {
        log.error(`mintPassCardSaleRoundParams2 not defined`)
        return
    }
