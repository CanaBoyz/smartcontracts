...
async function main() {
    log.header("Start MintPassCard Sale Round 4")
    const { mintPassCardSaleRoundParams4, cardSellerAddress } = meta.read()

    if (!mintPassCardSaleRoundParams4) {
        log.error(`mintPassCardSaleRoundParams4 not defined`)
        return
    }
