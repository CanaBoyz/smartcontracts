...
async function main() {
    log.header("Start MintPassCard Sale Round 3")
    const { mintPassCardSaleRoundParams3, cardSellerAddress } = meta.read()

    if (!mintPassCardSaleRoundParams3) {
        log.error(`mintPassCardSaleRoundParams3 not defined`)
        return
    }
