...
async function main() {
    log.header("Start MintPassCard Sale Round 5")
    const { mintPassCardSaleRoundParams5, cardSellerAddress } = meta.read()

    if (!mintPassCardSaleRoundParams5) {
        log.error(`mintPassCardSaleRoundParams5 not defined`)
        return
    }
